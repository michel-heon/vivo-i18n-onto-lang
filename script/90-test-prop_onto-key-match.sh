...
echo "= EVALUATION FOR $(basename $KEY_LIST_ONTO) AND $(basename $KEY_LIST_PROP) in ($THEME)================"
echo "= Word Count = ($(wc -l $KEY_LIST_ONTO))"
echo "= Word Count = ($(wc -l $KEY_LIST_PROP))"
echo "= DIFF Report ===" 
echo "      If the list is empty, it means that the keys of the property file and the ontology match"
#cat $KEY_LIST_ONTO $KEY_LIST_PROP | sort | uniq -u
diff  --suppress-common-lines -y -W 120 $KEY_LIST_ONTO $KEY_LIST_PROP | head -10
