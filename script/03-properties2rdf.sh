...
function process_extraction() {
    ...
    export TMP_ONTO_RESULT=$TMPDIR/$ONTO_FN.n3
    touch $TMPDIR/$ONTO_FN.n3
    build_indv $TMP_PROP_FN
    process_ftl
    cat $TMP_ONTO_RESULT | sort | uniq | grep -Ev "^$" > $ONTO_DATA/$SUB_REP/$ONTO_FN.n3
    echo "      Convert $ONTO_DATA/$SUB_REP/$ONTO_FN.n3 to TTL"
    func_nt2ttl.sh < $ONTO_DATA/$SUB_REP/$ONTO_FN.n3 > $ONTO_DATA_TTL/$SUB_REP/$ONTO_FN.ttl
    echo "      Done convert $ONTO_DATA/$SUB_REP/$ONTO_FN.n3 to TTL"
    echo "  Done [$KEY] !"
...
do
    ((LOOP_CTR=LOOP_CTR+1))
    #    _KEY=acct_created_email_html_text
